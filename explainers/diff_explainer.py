...
class DiffExplainer(Explainer):
    ...
    def explain_graph_task(self, args, train_dataset, test_dataset):
        ...
        # # 使用 DataParallel 将模型并行到多个 GPU
        # gnn_model = torch.nn.DataParallel(gnn_model, device_ids=[0, 1]).cuda()
        ...
        #     gnn_model = torch.nn.DataParallel(gnn_model, device_ids=[0, 1])
    ...
    def train(self, args, model, gnn_model, train_dataset, test_dataset):
        ...
        for epoch in range(args.epoch):
            ...
            for i, graph in enumerate(train_loader):
                ...
                # print("train_adj_b.shape",train_adj_b.shape)
                ...
                train_node_flag_b = train_adj_b.sum(-1).gt(1e-5).to(dtype=torch.float32)  # [bsz, N]   标记非零连接的节点。
                # all nodes that are not connected with others
                if isinstance(sigma_list, float):
                    sigma_list = [sigma_list]
                (train_x_b, train_ori_adj_b, train_node_flag_sigma, train_noise_adj_b, _) = gen_list_of_data_single(   #生成带噪声的邻接矩阵和其他相关数据
                    train_x_b, train_adj_b, train_node_flag_b, sigma_list, args
                )
                optimizer.zero_grad()
                ################################# denoise ############################################
                train_noise_adj_b_chunked = train_noise_adj_b.chunk(len(sigma_list), dim=0)     #根据噪声水平 sigma_list 切分数据
                ...
                score = []    #用于存储每个噪声水平下模型的输出得分
                masks = []    #用于存储每个噪声水平下的掩码
                for i, sigma in enumerate(sigma_list):      #对每个噪声水平进行计算掩码和模型的输出得分
                    mask = generate_mask(train_node_flag_sigma[i])
                    score_batch = model(
                        A=train_noise_adj_b_chunked[i].cuda(),
                        node_features=train_x_b_chunked[i].cuda(),
                        mask=mask.cuda(),
                        noiselevel=sigma,
                    )  # [bsz, N, N, 1]
                ...
                modif_r = sparsity(score, train_adj_b, mask)              #是对比模型输出与原始邻接矩阵 train_adj_b 的稀疏性   值
                remain_r = sparsity(score, train_adj_b, train_adj_b)      #是对比模型输出与自身的稀疏性    值
                ################################# loss ############################################
            ...
            if (epoch + 1) % args.verbose == 0:
                ...
                print(
                    (
                        ...
                        f"evaluation loss: {mean_test_loss} | "
                        f"evaluation fidelity drop: {mean_test_fid} | "
                        f"evaluation cf acc: {mean_test_acc} | "
                        f"evaluation average modification: {mean_test_sparsity} | "
                    )
                )
    ...
    def test_model(self, args, test_dataset):
        """Test the graph with the trained model"""
        test_losses = []
        test_acc = []
        test_fid = []
        test_sparsity = []
        ...
        for graph in test_loader:
            ...
            with torch.no_grad():
                ...
                for i, sigma in enumerate(sigma_list):
                    ...
                    score.append(score_batch)
            graph_batch_sub = tensor2graph(graph, score, mask)
            y_pred, y_exp = gnn_pred(graph, graph_batch_sub, self.model, ds=args.dataset, task=args.task)
            full_edge_index = gen_full(graph.batch, mask)
            score_b = torch.cat(score, dim=0).squeeze(-1).to(args.device)
            masktens = torch.cat(masks, dim=0).to(args.device)
            modif_r = sparsity(score, test_adj_b, mask)
